...
build_signflow() {
  ...
  if [[ ! -f "$INSTALL_DIR/package.pi.json" ]]; then
    die "package.pi.json missing — push latest SignFlow-PI and re-run install"
  fi

  log "Using package.pi.json (no Electron — avoids deprecated electron-builder deps)"
  if [[ -f "$INSTALL_DIR/package.json" && ! -f "$INSTALL_DIR/package.desktop.json" ]]; then
    cp "$INSTALL_DIR/package.json" "$INSTALL_DIR/package.desktop.json"
  fi
  cp "$INSTALL_DIR/package.pi.json" "$INSTALL_DIR/package.json"
  # Desktop package-lock.json pins Electron; must not be used on Pi
  ...
  bash -c "cd '$INSTALL_DIR' && npm install && node scripts/patch-pi-server.mjs && npm run build:pi-deploy"
